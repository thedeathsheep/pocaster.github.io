---
layout: post
title: Prompts for Cursor / AI 代码助手
...
mathjax: false
published: true
excerpt: 自用提示词备忘：高保真 HTML 原型、企业后台、落地页等；可直接粘贴到 Cursor 等工具里改词复用。
---
...
2.  **产品规划**：在提示中**设定角色为资深产品经理（约 20 年经验）**，统筹产品结构与信息架构，保证模块划分合理。
3.  **UI 设计**：在提示中**设定角色为资深 UI 设计师（约 30 年经验）**，输出需符合目标人群与移动端规范，使用现代组件与清晰层级，兼顾体验与视觉。
4.  **输出要求**：
...
我需要开发一个**企业内部项目管理系统**。
在提示中要求模型**扮演资深企业级应用设计师兼前端**，输出一套功能完整、易于理解的管理后台原型界面方案。
...
在提示中要求模型**扮演全栈营销向设计师兼前端**，快速产出高转化导向的单页落地页原型。
...
在提示中要求模型作为具备 10 年微前端与组件库经验的架构师兼前端，生成初步原型架构与示例页面。
...
在提示中要求模型作为具备 10 年游戏 UI/UX 与前端动画经验的开发者，设计一套有辨识度、动效清晰的 APP 原型。
...
在提示中要求模型作为资深市场分析师兼演示设计方向，生成 PPT 核心内容、结构大纲与版式建议。
...
在提示中要求模型作为 15 年经验量级的游戏制作人与策划，输出 GDD 大纲并填充核心设计段落。
...
在提示中要求模型作为资深数据科学家（约 10 年经验设定），设计分析模型并给出示例 SQL 与 Python 脚本骨架。
...
在提示中要求模型作为 12 年产品管理量级、偏策略与可视化的专家，生成报告核心结构、关键分析点与图表类型建议。
...
在提示中要求模型作为叙事与世界构建向资深角色，搭建叙事网络并梳理核心角色关系（可用 Mermaid 等文本图表达）。
...
在提示中要求模型作为大规模分布式方向首席架构师（经验设定），输出概念架构图（文本/Mermaid）与关键组件说明。
